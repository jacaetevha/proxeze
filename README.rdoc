...
The constructor will receive the type of the callback (one of #before, #before_all, #after, or #after_all),
the object, the arguments passed to the method, the method called, and a result (which is populated for #after and 
#after_all hooks). You need to define a #call method. Proxeze will create a new instance of the class every time
it needs to use it.
